  ...
  return (
    <>
      ...
      >
        ...
        {/* Mobile Navigation - Elevated above content */}
        {isMenuOpen && (
          <div className="md:hidden bg-gray-900/95 backdrop-blur-md border-t border-gray-700 shadow-2xl">
            <nav className="max-w-7xl mx-auto px-3 sm:px-4 pt-4 pb-6 space-y-3">
              {navItems.map((item) =>
                item.action ? (
                  <button
                    key={item.name}
                    ...
                    className="w-full text-left text-gray-300 hover:text-white hover:bg-gray-800/50 py-3 px-4 rounded-lg transition-all duration-200 font-medium block text-base"
                  >
                    {item.name}
                  </button>
                ) : (
                  <a
                    key={item.name}
                    href={item.href}
                    className="w-full block text-gray-300 hover:text-white hover:bg-gray-800/50 py-3 px-4 rounded-lg transition-all duration-200 font-medium text-base"
